...
import com.kyoofus.framework.security.custom.CustomAuthenticationProvider;
import com.kyoofus.framework.security.custom.CustomSecurityFilter;
import com.kyoofus.framework.security.formlogin.service.CustomUserDetailsService;
import com.kyoofus.framework.security.handlers.AuthFailHandler;
import com.kyoofus.framework.security.handlers.AuthSuccessHandler;
import com.kyoofus.framework.security.oauth2.services.CustomOAuth2UserService;
import com.kyoofus.framework.security.oauth2.services.CustomOidcUserService;
...
public class SecurityConfig extends SecurityConstatns {
    ...
    /** 정적자원에 대한 보안규칙을 정의한다.  */
    @Bean
    @Order(org.springframework.boot.autoconfigure.security.SecurityProperties.BASIC_AUTH_ORDER - 50 )
    ...
    /**  커스텀 인증 보안규칙을 정의한다. */
    @Bean
    @Order(org.springframework.boot.autoconfigure.security.SecurityProperties.BASIC_AUTH_ORDER - 40) // 순서가 filterStaticResources() 뒤에 오도록 순서를 지정해야 함
    public SecurityFilterChain apiFilterChain(HttpSecurity http, AuthenticationManager authManager) throws Exception {
        ...
        http
                .securityMatcher("/tutorial/**", "/api/v1/**")
                .authorizeHttpRequests(auth -> auth
